...
const recordSteps = [
  { key: "situation", title: "Situación", prompt: "¿Qué pasó? Describí el evento.", type: "textarea" },
  { key: "emotion", title: "Emoción", prompt: "¿Qué sentís? (Triste, ansioso, etc.) y evaluá la intensidad de 1 a 100.", type: "emotion" },
  { key: "thought", title: "Pensamiento Automático", prompt: "¿Qué te dijiste en ese momento? ¿Qué tan real lo sentís? (0-100).", type: "belief" },
  { key: "alternative", title: "Respuesta Alternativa", prompt: "¿Qué otra explicación más equilibrada existe?", type: "textarea" },
  { key: "result", title: "Resultado", prompt: "Reevaluá el pensamiento inicial y anotá cómo te sentís ahora.", type: "textarea" },
];
...
export default function Rumination() {
  ...
  const [recordData, setRecordData] = useState<Record<string, string | number>>({ intensity: 50, belief: 50 });
  const [thought, setThought] = useState("");
  const current = recordSteps[recordStep];
  const defusedThought = thought.trim() ? `Estoy teniendo el pensamiento de que ${thought.trim().toLowerCase()}` : "";
  ...
  if (view === "intro") {
    return (
      <div className="flex min-h-screen flex-col bg-resource-rumination-bg px-5 pt-12 pb-6 text-resource-rumination-accent safe-area-top">
        <button onClick={() => navigate("/herramientas")} className="mb-4 flex h-11 w-11 items-center justify-center rounded-full bg-card/75 shadow-sm">
          <ArrowLeft size={20} />
        </button>
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-1 flex-col items-center text-center">
          ...
          <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-[2rem] bg-card/75 shadow-sm shadow-resource-rumination-accent/10">
          ...
          <div className="mt-8 w-full space-y-3">
            <button onClick={() => { setRecordStep(0); setView("record"); }} className="flex w-full items-center gap-4 rounded-[2.5rem] border border-resource-rumination-accent/15 bg-card/75 p-5 text-left shadow-sm active:scale-[0.98]">
              ...
              <span><span className="block font-display text-base font-semibold">Registro de Pensamientos</span><span className="text-xs font-semibold opacity-65">Enfoque TCC</span></span>
            </button>
            <button onClick={() => setView("observer")} className="flex w-full items-center gap-4 rounded-[2.5rem] border border-resource-rumination-accent/15 bg-card/75 p-5 text-left shadow-sm active:scale-[0.98]">
              ...
              <span><span className="block font-display text-base font-semibold">El Observador</span><span className="text-xs font-semibold opacity-65">Enfoque ACT</span></span>
            </button>
          </div>
        </motion.div>
      </div>
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-resource-rumination-bg px-5 py-8 text-center text-resource-rumination-accent safe-area-top">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-sm">
          ...
          <div className="mx-auto mb-7 flex h-20 w-20 items-center justify-center rounded-full bg-card/80 shadow-xl shadow-resource-rumination-accent/10">
    ...
    return (
      <div className="relative flex min-h-screen flex-col overflow-hidden bg-resource-rumination-bg px-5 pt-14 pb-5 text-resource-rumination-accent safe-area-top">
        ...
        <div className="mb-5 flex items-center gap-3">
          <button onClick={goBack} className="flex h-11 w-11 items-center justify-center rounded-full bg-card/75 shadow-sm"><ArrowLeft size={20} /></button>
          <div><p className="font-display text-lg font-semibold">El Observador</p><p className="text-xs font-semibold text-resource-rumination-accent/65">Miralo pasar</p></div>
        </div>
        <div className="rounded-[2.5rem] border border-resource-rumination-accent/15 bg-card/70 p-5 shadow-sm backdrop-blur">
          <p className="mb-3 text-sm font-semibold leading-6 text-resource-rumination-accent/75">Escribí un pensamiento que te esté molestando.</p>
          <textarea value={thought} onChange={(e) => setThought(e.target.value)} placeholder="No voy a poder..." className="min-h-[92px] w-full resize-none rounded-[2rem] border border-resource-rumination-accent/15 bg-resource-rumination-bg/70 p-4 text-sm font-semibold outline-none placeholder:text-resource-rumination-accent/35 focus:ring-2 focus:ring-resource-rumination-accent/20" />
        </div>
        <div className="relative flex flex-1 items-center justify-center py-8">
          <AnimatePresence>
            {defusedThought && (
              <motion.div key={defusedThought} initial={{ opacity: 0, x: -34, y: 20, scale: 0.92 }} animate={{ opacity: 1, x: [ -20, 12, -6 ], y: [ 16, -18, 8 ], scale: 1 }} exit={{ opacity: 0, x: 60 }} transition={{ duration: 8, repeat: Infinity, repeatType: "mirror", ease: "easeInOut" }} className="max-w-[300px] rounded-[2.5rem] border border-resource-rumination-accent/15 bg-card/80 px-6 py-5 text-center shadow-xl shadow-resource-rumination-accent/10 backdrop-blur">
                <Cloud className="mx-auto mb-2" size={28} weight="duotone" />
                <p className="text-base font-semibold leading-7 text-resource-rumination-accent/80">{defusedThought}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        <p className="mb-4 rounded-[2rem] bg-card/60 p-4 text-center text-sm font-semibold leading-6 text-resource-rumination-accent/70">No sos tu pensamiento. Sos quien lo observa pasar. Dejalo ir como una nube en el cielo.</p>
        <button onClick={finish} className="rounded-[2.5rem] bg-resource-rumination-accent py-4 font-display text-base font-semibold text-primary-foreground shadow-lg shadow-resource-rumination-accent/20 active:scale-[0.98]">Finalizar</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-resource-rumination-bg px-5 pt-14 pb-5 text-resource-rumination-accent safe-area-top">
      ...
      <p className="mb-5 rounded-[2rem] bg-card/60 p-4 text-sm font-semibold leading-6 text-resource-rumination-accent/70">Cuando sientas una emoción desagradable, anotá lo que está pasando para evaluarlo mejor.</p>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={current.key} initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -28 }} className="flex-1">
          ...
          <div className="mt-6 rounded-[2.5rem] border border-resource-rumination-accent/15 bg-card/70 p-5 shadow-sm backdrop-blur">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <button onClick={goBack} className="rounded-[2.5rem] border border-resource-rumination-accent/20 bg-card/70 py-3.5 font-display text-sm font-semibold shadow-sm active:scale-[0.98]">Atrás</button>
