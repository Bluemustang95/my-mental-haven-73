...
export default function Onboarding() {
  ...
  const handleFinish = () => {
    // TODO: save to profile via Supabase
    localStorage.setItem("resma_onboarding", JSON.stringify({ lifeStage, areas, feelings, treatment }));
    navigate("/");
  };
        ...
        >
          ...
          {step === 0 && (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <h1 className="mb-3 font-display text-2xl font-semibold leading-tight">
                Tu espacio seguro para cuidar tu salud mental
              </h1>
              <p className="mb-10 max-w-xs text-sm text-muted-foreground">
                Herramientas de autogestión, acompañamiento y acceso a tratamiento profesional.
              </p>
              <button
                onClick={() => setStep(1)}
                className="flex items-center gap-2 rounded-2xl bg-primary px-8 py-3 font-display text-sm font-medium text-primary-foreground transition-transform active:scale-95"
              >
                Comenzar
                <ArrowRight size={16} />
              </button>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {areasOfInterest.map((area) => (
                  <button
                    key={area}
                    onClick={() => toggleItem(areas, setAreas, area)}
                    className={cn(
            ...
            <div>
              ...
              <div className="space-y-2">
                {recentFeelings.map((f) => (
                  <button
                    key={f.value}
                    onClick={() => toggleItem(feelings, setFeelings, f.value)}
                    className={cn(
                      "w-full rounded-2xl border p-3.5 text-left font-display text-sm transition-all",
                      feelings.includes(f.value)
      ...
      {step > 0 && (
        <div className="mt-6 flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-1 font-display text-sm text-muted-foreground"
          >
            <ArrowLeft size={14} />
            Atrás
          </button>
