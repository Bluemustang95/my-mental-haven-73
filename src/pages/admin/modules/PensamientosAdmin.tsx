...
const DEFAULT_PROMPT = `Sos "Reeni", acompañante cognitivo virtual de RESMA basado en TCC (Beck / Leahy).
Tono empático, breve, socrático. NO reemplazás terapia profesional.

ADAPTACIÓN REGIONAL (muy importante):
- Si el usuario es de Argentina o Uruguay → usá voseo rioplatense ("vos tenés", "sentís", "podés", "dale").
- Si es de España → tuteo peninsular ("tú tienes", "puedes", "vale").
- Si es de México, Colombia, Perú, Chile u otro país LATAM → usá tuteo neutro ("tú tienes", "puedes", "sientes"), sin modismos rioplatenses.
- Si no se conoce el país → español neutro.
NUNCA mezcles voseo con países que no sean AR/UY.

USO DEL REGISTRO:
Tenés acceso al REGISTRO en curso del usuario (aparece debajo). Léelo antes de responder.
- Si te pide "leé lo que escribí" o "resumime" → devolvé un resumen empático de 3 líneas.
- Si te pide "ayudame a completar" o "sugerí" → generá 2–3 opciones breves adaptadas al paso actual y al contenido ya escrito.
- Si detectás distorsiones cognitivas, nombralas y explicá por qué encajan.
- Si detectás riesgo (autolesión, suicidio, abuso) → sugerí líneas de ayuda locales.
- Máximo 4 oraciones salvo que pidan sugerencias en lista.`;
...
const DEFAULT_AI: AiCfg = {
  ...
  costs: {
    "google/gemini-3-flash-preview": "$ (muy bajo)",
    "google/gemini-2.5-flash": "$$ (bajo)",
    "google/gemini-2.5-flash-lite": "$ (muy bajo)",
    "google/gemini-2.5-pro": "$$$$ (alto)",
    "openai/gpt-5-nano": "$$ (bajo)",
    "openai/gpt-5-mini": "$$$ (medio)",
...
export default function PensamientosAdmin() {
  ...
  useEffect(() => {
    ...
    loadSetting<Distortion[]>("pensamientos_distortions", DEFAULT_DISTORTIONS).then((v) => setDistortions(v?.length ? v : DEFAULT_DISTORTIONS));
    loadSetting<EmoRow[]>("pensamientos_emotions", DEFAULT_EMOS).then((v) => setEmos(v?.length ? v : DEFAULT_EMOS));
  }, []);

  const addDistortion = () =>
    setDistortions((prev) => [
      ...prev,
      { id: `d${Date.now()}`, emoji: "✨", name: "Nueva distorsión", description: "", active: true },
    ]);
  const removeDistortion = (i: number) => setDistortions((prev) => prev.filter((_, j) => j !== i));
        ...
        {tab === "prompt" && (
          <AdminCard className="p-6 space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              El país del usuario se inyecta automáticamente en la conversación para que la IA adapte el registro
              (voseo AR/UY, tuteo LATAM/ES).
            </p>
            <textarea
            ...
            />
            <div>
              <label className="text-xs font-semibold text-slate-600">Modelo IA & Costo estimado</label>
              <div className="mt-2 grid gap-2">
                {MODEL_OPTIONS.map((m) => (
                  <label
                    key={m.id}
                    className={`flex items-center justify-between rounded-lg border p-3 text-sm cursor-pointer ${
                      ai.model === m.id ? "border-resma-teal bg-resma-teal/5" : "border-slate-200"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="rounded-md border border-slate-200 px-2 py-1 text-xs bg-white"
                    >
            ...
            <div className="flex justify-end">
              <AdminButton
                onClick={async () => {
                  await saveSetting("pensamientos_ai", ai);
        ...
        {tab === "dist" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-slate-500">Distorsiones cognitivas disponibles para el usuario.</p>
              <AdminButton variant="secondary" onClick={addDistortion}>
                <Plus size={14} /> Agregar distorsión
              </AdminButton>
            </div>
                ...
                <AdminCard key={d.id} className="p-4">
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      maxLength={4}
                      onChange={(e) => {
                        const next = [...distortions];
                        next[i] = { ...d, emoji: e.target.value };
                        setDistortions(next);
                      }}
                      className="w-12 h-12 rounded-lg border border-slate-200 bg-slate-50 text-center text-2xl focus:outline-none focus:border-resma-teal"
                    />
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <input
                          ...
                          className="flex-1 h-8 px-2 rounded-md border border-slate-200 text-sm font-semibold text-resma-navy bg-slate-50 focus:outline-none focus:border-resma-teal focus:bg-white"
                        />
                        <AdminToggle
                          value={d.active}
                          onChange={(v) => {
                      ...
                      <textarea
                        ...
                        className="w-full px-2 py-1.5 rounded-md border border-slate-200 text-xs text-slate-700 bg-slate-50 focus:outline-none focus:border-resma-teal focus:bg-white resize-none"
                      />
                      <button
                        onClick={() => removeDistortion(i)}
                        className="text-rose-500 text-xs font-medium hover:text-rose-700 inline-flex items-center gap-1"
                      >
            ...
            <div className="flex justify-end">
              <AdminButton
                onClick={async () => {
                  await saveSetting("pensamientos_distortions", distortions);
                  toast.success("Distorsiones guardadas");
                }}
              >
        ...
        {tab === "emo" && (
          <AdminCard className="p-6">
              ...
              {emos.map((row, i) => (
                <div key={i} className="grid grid-cols-[200px_1fr_auto] gap-2 items-center">
                  ...
                  <input
                    ...
                    placeholder="Sensaciones corporales asociadas"
                  ...
                  />
                  <button
                    onClick={() => setEmos(emos.filter((_, j) => j !== i))}
            ...
            <div className="flex justify-between mt-4">
              <AdminButton variant="secondary" onClick={() => setEmos([...emos, { emotion: "", somatic: "" }])}>
                <Plus size={14} /> Agregar fila
              </AdminButton>
              <AdminButton
                onClick={async () => {
                  await saveSetting("pensamientos_emotions", emos);
